...
template<typename LARGE_BITSET>
void DeBruijnGraph<LARGE_BITSET>::MakeDBG(KmerSet &seedkmer,uint64_t filtersize ,uint8_t numhashes){

    for (auto itr=seedkmer.begin();itr!=seedkmer.end();++itr){
        std::cout<<"search new read"<<"\n";
        LARGE_BITSET fw_kmer=GetFirstKmerForward<LARGE_BITSET>(*itr);
